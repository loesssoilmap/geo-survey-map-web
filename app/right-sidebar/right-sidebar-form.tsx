...
import { Category, updateApiClient, useCreateSurvey } from 'geo-survey-map-shared-modules'
...
export const RightSidebarForm: React.FC<RightSidebarFormProps> = ({ handleClose }) => {
	...
	const handleAddMarker = async () => {
		...
		if (response.status === 200) {
			toast.success('Znacznik został poprawnie dodany.')
		} else {
			toast.error('Ooops! Coś poszło nie tak. Spróbuj ponownie później.')
		}
...
const Categories = () => {
	...
	return (
		<React.Fragment>
			<small className="font-bold text-gray">Kategorie</small>
...
const AffectedArea = () => {
	...
	return (
		<React.Fragment>
			<small className="font-bold text-gray">Dotknięty obszar</small>
			<input
